javascript:(function(){
  ...
  myTimeout = setTimeout(getVideoEl, 200);

    function init() { 
    if (document.getElementById('vidNotePar')!=null) return;
    var scriptText = '\
  ...
  for(var k in notes){ \
  el.innerHTML+="<tr style=\'cursor:pointer;\' onclick=seek("+parseFloat(k)+")><td >"+k+"</td><td>"+notes[k]+"</td></tr>"; \
  } el.innerHTML+="</table>";\
  } \
  ...
  msgEl.innerHTML = '<table id="notes" style="font-size:14px;width:100%;"></table>'; 
